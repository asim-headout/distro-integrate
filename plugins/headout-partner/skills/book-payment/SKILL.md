...
5. **Assemble** in canonical order, wiring the **CTA state machine** and the create→charge→capture flow exactly.
...
## Submit → charge → capture flow (STRICT)
...
2. **Charge on the partner PSP:** tokenize via the PSP (hosted fields/SDK) — raw card data never touches app state. Branch on the PSP's next action:
...
3. **Capture + confirm:** on PSP success, **capture the Headout booking (update)**, then **get booking** (poll with a sane timeout/backoff) until status is confirmed → navigate to the confirmation page.
...
| method valid & ready | **"Pay {total}"** | disable CTA, create Headout booking (UNCAPTURED), charge on the PSP (3DS/redirect), then capture + confirm |
...
- [ ] Submit → create (UNCAPTURED) → PSP charge (inline/3DS/redirect) → capture (update) → get/confirm; failure/timeout → retry page that preserves the cart and offers "Try again".
