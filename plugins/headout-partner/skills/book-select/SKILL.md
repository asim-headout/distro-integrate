...
- **Time section:** hidden until an option is selected (or a price fetch is in progress → show a shimmer). A single available slot renders as an auto-selected read-only row, not a dropdown; multiple slots render as a list showing up to ~6 with a "Show more" to expand.
...
**Step A — reuse an existing design system first.** Search the partner repo for one (`design-system/`, `ui/`, `components/ui/`, an exported `Box`/`Text`/`Button`, a `panda.config.*`/`tailwind.config.*`/theme-tokens file). If found, **map each role to the partner's component and tokens — build no new primitives.**
...
- **Calendar (CalendarModal):** build the date picker with the partner's design system; `ui-calendar` (`/ui-calendar`) is an **optional fallback reference** for its structure/behavior (dual-month grid; selected / min-price / unavailable states) when the partner has no date picker — it imposes no Headout dimensions or tokens. Wire it as the CalendarModal opened by the "More dates" button on the DateStrip.
...
- **Calendar modal:** a modal/sheet (not a native date input unless the partner's design system requires it); two months visible on desktop, one on mobile; day cells show available/disabled state and starting selling price.
