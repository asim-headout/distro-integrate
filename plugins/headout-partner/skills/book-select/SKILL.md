...
## Pixel-exact component spec (source: zapdos)

> Build once into `ui-components/` and reuse across checkout/payment. These measurements come directly from the canonical Headout zapdos production implementation.

### Step breadcrumb / StepHeader
Shown at the top of the page: `1. {product name} › 2. Tickets › 3. Confirm & pay`. Step 2 reads **"Select seats"** for seatmap/SVG-zone modes. Active step highlighted with the primary accent; completed steps are muted/linked; future steps are grey.

### DateCard (individual date pill in the strip)
| Property | Value |
|---|---|
| Size | `3.5rem × 4.5rem` (56 × 72px) |
| Border-radius | `10px` |
| Border | `1px solid #E2E2E2` (`core.grey.300`) |
| Day-of-week text | `ui.label.small`; bg `semantic.surface.light.grey.2` |
| Date number text | `heading.medium` |
| Price text | `ui.label.regular.heavy`; color `core.warningred.700` (reddish-orange) |
| Selected bg | `FLOATING_PURPS` ≈ `#e6d1ff` (light purple) |
| Selected border | `#8000ff` (`PURPS`) |
| Unavailable | muted grey text; `pointer-events: none` |
| Gap between pills | `0.75rem` |
| Scroll | Horizontal; `-webkit-overflow-scrolling: touch`; scrollbar hidden |
| Animation | `transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)` on select |

**"More dates" button** — opens CalendarModal (see `ui-calendar` skill); excluded from `useCaptureClickOutside`; QA marker: `calendar-strip-pricedate`.

---

### VariantDetailsCard (horizontal-scroll carousel mode — multi-variant)
| Property | Value |
|---|---|
| Width | `9.5rem` (152px) |
| Min-width | `9.5rem` |
...
| Border | `1px solid #E2E2E2`; border-bottom `3px solid #E2E2E2` |
| **Selected** bg | `#e6d1ff` (`FLOATING_PURPS`) |
| **Selected** border | `#8000ff` (`PURPS`) |
| **Unavailable** bg | `#F8F8F8` (`G8`); border `#E2E2E2`; text `#888888` (`G4`); `cursor: default` |
| Hover (desktop) | `translateY(-2px)`; `transition: all 150ms ease-in-out` |
| Active press | `scale(0.97)` |
| Price font | `HEADING_PRODUCT_CARD` |
| Strike-price font | `SUBHEADING_XS`; color `#888888`; `text-decoration: line-through` |
| Sold-out label | `UI_LABEL_MEDIUM_HEAVY`; color `#888888`; margin-top `1rem` |

**Radio button inside card:**
- Size: `20px × 20px`; border-radius `50%`; border `1px solid #C4C4C4` (G5)
- Selected: bg `#8000ff`; no border; inner dot `8px × 8px` white

---

### SingleVariantCard (desktop full-width layout — single variant or expanded detail)
| Property | Value |
|---|---|
| Border-radius | `radius.12` (12px) |
| Box-shadow | `0px 0px 0px 1px #E2E2E2` |
| Background | `semantic.surface.light.white` |
| Margin-top | `space.10` (10px) |
| **Left section** (title / descriptors / price) | Width `316.8px`; padding `16px 16px 20px 16px`; gap `space.12`; border-right `1px solid semantic.dividers.dark` |
| **Right section** (inclusions bullets) | Width `443.2px`; padding `16px`; gap `space.8`; text `para.medium`; color `semantic.text.grey.1` |
| Title text | `heading.regular`; color `semantic.text.grey.1` |
| Mobile | full-width; flex-column; box-shadow `0px -4px 0px 0px #E2E2E280 inset, 0px 2px 6px -2px #E2E2E2` |

**Inclusions/Exclusions bullets:**
- `✓` check in `core.okaygreen.700` / `✗` cross in `semantic.text.destructive`
- "Read more" link when > ~5 items (opens a modal)

**CTA button inside card (state button):**
- States: **"Select"** (outlined) → **"Selected ✓"** (purple filled) → **"Sold out"** (grey, disabled)
- Color: `purps` variant from `@headout/eevee` Button; size `medium`
...
| Property | Value |
|---|---|
| Background | `semantic.surface.light.success.1` (light green) |
| Border-radius | `radius.4` (4px) |
| Box-shadow | `0px -1px 0px 0px rgba(176, 227, 199, 1) inset` (green inset top border) |
| Padding | `3px 8px 5px` |
| Gap | `4px` (icon + text) |
| Icon | `MoneyBagStarFilled` from `@headout/onix`; `14px × 14px`; color `core.okaygreen.700` |
| Text | `ui.label.small.heavy`; color `core.okaygreen.800` |

Position the badge **above** the option card title. Shown only on variants where `tours.length > 1`.

---

### Time slot list ("Pick a time")
| Property | Value |
|---|---|
| Layout | `display: grid`; `row-gap: 0.75rem` |
| Pagination | Max **6 slots** visible; "Show more" expands |
| Container padding-bottom | `3.875rem` (to clear sticky bottom bar on mobile) |
| Single slot | Renders as auto-selected read-only row (not a dropdown) |
| Multi-slot | Renders as a paginated list of selectable time buttons |
| Validation | "Select a time slot to continue" highlight shown until slot is chosen |

---

### BookingItineraryCard (sticky summary card — desktop right column)
| Property | Value |
|---|---|
| Border | `1px solid #E2E2E2` (`GREY_DS.G6`) |
| Border-radius | `12px` |
| Background | `#fff` |
| Sticky top | `6.125rem` (98px) |
| Z-index | `5` |
| Width | `~24rem` (fixed right column) |

**Internal rows (selection display):**
- Product banner (image + name) at top
- Date row with edit icon (`✎`)
- Time row with edit icon
- Variant/option row with edit icon
- Price row (selling price, large; strike-through original if discounted)
- **State CTA button** — full-width; `purps` color; text driven by state machine (see above)
...
| Z-index | `100` |
| Layout | Left: product name + selected date (two-line); Right: state CTA button |
...
| Property | Value |
|---|---|
| Component | `Button` from `@headout/eevee`; `variant="primary"`; `color="purps"`; `size="medium"` |
| Active bg | `#8000ff` (`PURPS`) |
| Hover bg | `#6600cc` (`PURPS_DARK`) |
| Text color | `semantic.surface.light.white` |
| Disabled bg | `#C4C4C4` (`G5`) |
...
| Role | Token / Hex |
|---|---|
| Primary action (purple) | `PURPS` = `#8000ff` |
| Primary hover | `PURPS_DARK` = `#6600cc` |
| Selected bg (light purple) | `FLOATING_PURPS` ≈ `#e6d1ff` |
| Sold-out / disabled text | `G4` = `#888888` |
| Card border | `G6` = `#E2E2E2` (`core.grey.300`) |
| Success green text | `core.okaygreen.800` |
| Success green bg | `semantic.surface.light.success.1` |
| Price accent | `core.warningred.700` (reddish-orange, used for date prices) |
