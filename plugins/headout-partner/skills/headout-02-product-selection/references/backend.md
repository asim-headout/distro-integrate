...
- Model `PER_PERSON` and `PER_GROUP` pricing separately; compute customer-facing totals from
  ...
  inventory fetch through to booking. Keep `netPrice` internal for Headout booking payloads and
  reconciliation; never expose it to the FE or use it as the customer charge.
...
- Expose availability (dates/slots) and per-variant pricing as mapped customer fields:
  `sellingPrice` from `headoutSellingPrice` and optional strike `originalPrice`. Keep `netPrice` in
  the server/order layer for booking-create and reconciliation. **Headout inventory returns no
